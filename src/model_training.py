...
class ModelTraining:
    def __init__(self, data_path):
        ...
        self.experiment = comet_ml.Experiment(
            ...
            project_name="anime-hybrid-recommender-system",
            workspace="parent folder name of project name in comet website"
        )
        ...
        try:
            ...
            def lrfn(epoch):
                if epoch < ramup_epochs:
                    return (max_lr - start_le) / ramup_epochs * epoch + start_le
    ...
    def extract_weights(name, layer_name, model):
        try:
            weight_layer = model.get_layer(layer_name)
            weights = weight_layer.get_weights()[0]
            weights = weights/np.linalg.norma(weights, axis=1).reshape((-1, 1))
