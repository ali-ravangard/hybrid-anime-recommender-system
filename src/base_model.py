from tensorflow.keras.modles import Model
...
class BaseModel:
    ...
    def recommenderNet(self, n_users, n_animes):
        try:
            ...
            model = Model(input=[user, anime], output=x)
            model.compile(loss=self.config["model"]["loss"],
                          optimizer=self.config["model"]["optimizer"],
                          metrics=self.config["model"]["metrics"],)
            
            logger.info("Model created succcessfully")
            return model
        
        except Exception as e:
            logger.error(f"Error occured during model architecture")
